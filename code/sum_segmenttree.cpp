...
void BuildTree(int node,int start,int end,int A[],int N){
    ...
    if(start==end){
       ...
       // printf("M[%d]=%d\n",node,M[node]);
        ...
        M[node]=M[2*node+1] + M[2*node+2];
      //printf("M[%d]=%d\n",node,M[node]);
    }
}

int RSQ(int node,int start,int end,int qs,int qe){
    if(qs<=start && qe>=end)
        return M[node];
    else if(qs>end || qe<start)
        return -1;
    int q1 = RSQ(2*node+1,start,(start+end)/2,qs,qe);
    int q2 = RSQ(2*node+2,(start+end)/2+1,end,qs,qe);
    if(q1==-1)
        return q2;
    else if(q2==-1)
        return q1;
